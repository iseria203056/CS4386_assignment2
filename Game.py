            ...
            while(self.gamestate==1):
                ...
                while i<2 and (self.gamestate==1):
                    ...
                    if self.players_list[i].draw(self.gametable):
                        ...
                        break
                    tic = time()
                    return_from_drop=self.players_list[i].drop(self.gametable,self.players_list[1-i])
                    toc = time()
                    if (toc-tic)>TIME_LIMIT:
                        print("Timed out for drop(), game over.")
                        print("player",i+1,"loses due to time-out")
                        return False

                    if return_from_drop==False:
                        self.gametable=0
                    ...
                    if True:
                        k=1-i
                        tic1 = time()
                        return_value=self.players_list[k].action(self.gametable,self.players_list[1-k])
                        toc1 = time()
                        print("--------------action time",toc1-tic1)
                        if (toc1-tic1)>TIME_LIMIT:
                            print("Timed out for action(), game over.")
                            print("player",k+1,"loses due to time-out")
                            return False
                        ...
                        if return_value==True:
                            ...
                            toc2 = time()
                            print("--------------drop time after action",toc2-tic2)
                            if (toc2-tic2)>TIME_LIMIT:
                                print("Timed out for drop() after action(), game over.")
                                print("player",k+1,"loses due to time-out")
                                return False
                            
                        elif return_value==-1:
                            print("invalid return value from action.Game ends!")
                            return False
                    i += 1
            for p in self.players_list:
                p.reset()
...
class Player():
    ...
    def updateAI(self,gametable,opponent):
        ...
        if self.language=="PYTHON" or self.language=="Human":
            ...
            int_arr2n = int_arr2*23
            cpp_a1 = int_arr2n()
            for i in range(l1):
                for j in range(2):
                    cpp_a1[i][j]=ctypes.c_int(python_a1[i][j])
            for i in range(l1,23):
                for j in range(2):
